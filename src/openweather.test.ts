...
it("gets a promise with the next forecast", () => {
  const config: openweather.Config = {
    ...
    requester: {
      get: (options: request.Options): Promise<request.Response> => {
        ...
        const content = fs.readFileSync(
          "fixtures/openweather/paris-fr-valid-api-key.json",
          { encoding: "utf8" }
        );
  ...
  return openweather.forecast(config, "paris", "fr").then(resp => {
